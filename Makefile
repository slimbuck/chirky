...
WEB_COMMON = web/host.c src/rect_renderer.c src/asset_store.c
WEB_HEADERS = $(wildcard include/*.h src/*.h)
...
$(TARGET): $(SOURCES) $(wildcard include/*.h src/*.h)
	mkdir -p $(@D)
...
build/performance-benchmark: tools/performance_benchmark.c $(SOURCES) $(wildcard src/*.h include/*.h)
	mkdir -p $(@D)
	$(CC) $(CPPFLAGS) $(CFLAGS) -ffunction-sections -fdata-sections tools/performance_benchmark.c src/input_bindings.c $(PLATFORM_SOURCES) -Wl,--gc-sections -ldl $(LDLIBS) -o $@

test: $(GAME_TARGETS)
	mkdir -p build
	$(CC) $(CPPFLAGS) $(CFLAGS) tests/asset_store.c src/asset_store.c -pthread -o /tmp/asset-store-test
	/tmp/asset-store-test
	$(CC) $(CPPFLAGS) $(CFLAGS) tests/audio_mixer.c -pthread -ldl -o /tmp/audio-mixer-test
	/tmp/audio-mixer-test
	$(CC) $(CPPFLAGS) $(CFLAGS) tests/audio_samples.c src/asset_store.c -pthread -ldl -o /tmp/audio-samples-test
	/tmp/audio-samples-test
	$(CC) $(CPPFLAGS) $(CFLAGS) tests/texture_splash.c -o /tmp/texture-splash-test
	/tmp/texture-splash-test
	$(CC) $(CPPFLAGS) $(CFLAGS) tests/platform_runtime.c src/asset_store.c -pthread -ldl -lm -o /tmp/platform-runtime-test
	...
	$(CC) $(CPPFLAGS) $(CFLAGS) -ffunction-sections -fdata-sections tests/host_runtime.c src/input_bindings.c src/rect_renderer.c $(PLATFORM_SOURCES) -pthread -Wl,--gc-sections -ldl -l:libGLESv2.so.2 -o /tmp/host-runtime-test
